...
public class EdgeConvertGUI {
   ...
   private EdgeField currentDTField, currentDRField1, currentDRField2; // pointers to currently selected field(s) on
                                                                       // Define Tables (DT) and Define Relations (DR)
                                                                       // screens
   private static boolean readSuccess = true; // this tells GUI whether to populate JList<String> components or not
   ...
   static JMenuItem jmiDTOpenEdge, jmiDTOpenSave, jmiDTSave, jmiDTSaveAs, jmiDTExit, jmiDTOptionsOutputLocation,
   ...
   static DefaultListModel<String> dlmDRTablesRelations, dlmDRTablesRelatedTo, dlmDRFieldsTablesRelations,
   ...
   public void showGUI() {
      try {
         ...
         logger.debug("exception caught on line 88:\n" + e.getMessage());
         System.out.println("Error setting native LAF: " + e);
      }
      logger.info("GUI instantiation done, GUI screen methods running");
      createDTScreen();
      createDRScreen();
   } // showGUI()
   ...
   public void createDTScreen() {// create Define Tables screen
      ...
      logger.info("Create tables frame setup done");
      ...
      dlmDTTablesAll = new DefaultListModel<String>();
      jlDTTablesAll = new JList<String>(dlmDTTablesAll);
      ...
      dlmDTFieldsTablesAll = new DefaultListModel<String>();
      jlDTFieldsTablesAll = new JList<String>(dlmDTFieldsTablesAll);
      ...
      jfDT.validate();
      logger.info("Define Tables screen ready");
   } // createDTScreen
   ...
   public void createDRScreen() {
      ...
      dlmDRTablesRelations = new DefaultListModel<String>();
      jlDRTablesRelations = new JList<String>(dlmDRTablesRelations);
      ...
      dlmDRFieldsTablesRelations = new DefaultListModel<String>();
      jlDRFieldsTablesRelations = new JList<String>(dlmDRFieldsTablesRelations);
      ...
      dlmDRTablesRelatedTo = new DefaultListModel<String>();
      jlDRTablesRelatedTo = new JList<String>(dlmDRTablesRelatedTo);
      ...
      dlmDRFieldsTablesRelatedTo = new DefaultListModel<String>();
      jlDRFieldsTablesRelatedTo = new JList<String>(dlmDRFieldsTablesRelatedTo);
      ...
      logger.info("Define Relations screen ready");
   } // createDRScreen
   ...
   private void enableControls() {
      ...
      logger.info("GUI Controls enabled");
   }
   ...
   private void disableControls() {
      ...
      logger.info("GUI controls disabled");
   }

   private void clearDTControls() {
      jlDTTablesAll.clearSelection();
      jlDTFieldsTablesAll.clearSelection();
      logger.info("Define tables controls cleared");
   }
   ...
   private void clearDRControls() {
      ...
      jlDRFieldsTablesRelations.clearSelection();
      jlDRFieldsTablesRelatedTo.clearSelection();
      logger.info("Define relations controls cleared");
   }
   ...
   private void saveAs() {
      ...
      if (returnVal == JFileChooser.APPROVE_OPTION) {
         ...
         if (saveFile.exists()) {
            logger.info("File found, confirming overwrite");
            int response = JOptionPane.showConfirmDialog(null, "Overwrite existing file?", "Confirm Overwrite",
                  JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE);
            if (response == JOptionPane.CANCEL_OPTION) {
               logger.warn("Save aborted due to a file of that name already existing");
               return;
            }
         }
         if (!saveFile.getName().endsWith("sav")) {
            logger.warn("File of type other than 'sav' detected");
   ...
   private void writeSave() {
      if (saveFile != null) {
         try {
            ...
            logger.info("Wrote identification line");
            // write the tables
            pw.println("#Tables#");
            for (int i = 0; i < tables.length; i++) {
               pw.println(tables[i]);
               logger.info("Wrote table index " + i);
            }
            // write the fields
            pw.println("#Fields#");
            for (int i = 0; i < fields.length; i++) {
               pw.println(fields[i]);
               logger.info("Wrote field index " + i);
            }
            // close the file
            pw.close();
         } catch (IOException ioe) {
            System.out.println(ioe);
            logger.error("IO exception caught on line 941:\n" + ioe.getMessage());
         }
         dataSaved = true;
      }
      logger.debug("Savefile = null; no save written");
   }
   ...
   private void setOutputDir() {
      ...
      alProductNames = new ArrayList<String>();

      returnVal = jfcOutputDir.showOpenDialog(null);

      if (returnVal == JFileChooser.CANCEL_OPTION) {
         logger.info("Aborting output directory change");
         return;
      }
      ...
      if (alProductNames.size() == 0) {
         ...
         logger.warn("No valid output definition files in " + outputDir);
         return;
      }
   ...
   private String displayProductNames() {
      ...
      logger.info("The following products found:\n" + sb.toString());
      return sb.toString();
   }
   ...
   private void getOutputClasses() {
      ...
      try {
         ...
         logger.debug("Instantiation exception caught:\n" + ie.getMessage());
         ie.printStackTrace();
      } catch (ClassNotFoundException cnfe) {
         logger.debug("Class not found exception caught:\n" + cnfe.getMessage());
         cnfe.printStackTrace();
      } catch (IllegalAccessException iae) {
         logger.debug("Illegal access exception caught:\n" + iae.getMessage());
         iae.printStackTrace();
      } catch (NoSuchMethodException nsme) {
         logger.debug("No such method exception caught:\n" + nsme.getMessage());
         nsme.printStackTrace();
      } catch (InvocationTargetException ite) {
         logger.debug("Invocation target exception caught:\n" + ite.getMessage());
         ite.printStackTrace();
      }
      if (alProductNames.size() > 0 && alSubclasses.size() > 0) { // do not recreate productName and objSubClasses
         ...
         logger.warn("No valid files in new path");
      }
   }
   ...
   private String getSQLStatements() {
      ...
      if (response == null) {
         logger.warn("Get SQL statements cancelled");
      ...
      for (selected = 0; selected < productNames.length; selected++) {
         if (response.equals(productNames[selected])) {
            logger.info("Product in use: " + productNames[selected]);
            break;
         }
      }
   ...
   private void writeSQL(String output) {
      ...
      if (databaseName.equals("")) {
         logger.warn("No database set, aborting write");
         return;
      }
      ...
      if (returnVal == JFileChooser.APPROVE_OPTION) {
         ...
         try {
            ...
            logger.debug("IO exception caught:\n" + ioe.getMessage());
            System.out.println(ioe);
         }
      }
   }
   ...
   class EdgeRadioButtonListener implements ActionListener {
      public void actionPerformed(ActionEvent ae) {
         logger.info("EdgeRadioButtonListener triggered");
   ...
   class EdgeWindowListener implements WindowListener {
      public void windowActivated(WindowEvent we) {
         logger.debug("EdgeWindowListener: window activated");
      }

      public void windowClosed(WindowEvent we) {
         logger.debug("EdgeWindwoListener: window closed");
      }

      public void windowDeactivated(WindowEvent we) {
         logger.debug("EdgeWindowListener: window deactivated");
      }

      public void windowDeiconified(WindowEvent we) {
         logger.debug("EdgeWindowListener: window deiconified");
      }

      public void windowIconified(WindowEvent we) {
         logger.debug("EdgeWindowListener: window iconified");
      }

      public void windowOpened(WindowEvent we) {
         logger.debug("EdgeWindowListener: window opened");
      }

      public void windowClosing(WindowEvent we) {
         logger.info("Window closing");
         if (!dataSaved) {
            logger.warn("Unsaved data detected");
            ...
            if (answer == JOptionPane.YES_OPTION) {
               if (saveFile == null) {
                  logger.info("Saving data to save file before close");
                  saveAs();
               } else {
                  logger.info("No previous save file found, saving before close");
               }
               writeSave();
            }
            if ((answer == JOptionPane.CANCEL_OPTION) || (answer == JOptionPane.CLOSED_OPTION)) {
               ...
               logger.info("Aborting exit due to unsaved data");
               return;
            }
         }
         logger.info("Exiting...");
   ...
   class CreateDDLButtonListener implements ActionListener {
      public void actionPerformed(ActionEvent ae) {
         logger.info("CreateDDL process starting...");
         while (outputDir == null) {
            logger.info("No valid output directory found, attempting to set one now");
            JOptionPane.showMessageDialog(null,
         ...
         if (sqlString.equals(EdgeConvertGUI.CANCELLED)) {
            logger.info("CreateDDL process canceled, aborting...");
            return;
         }
         logger.info("Beginning writeSQL");
         writeSQL(sqlString);
      }
   }
   ...
   class EdgeMenuListener implements ActionListener {
      public void actionPerformed(ActionEvent ae) {
         logger.info("Edge menu action performed");
         int returnVal;
         if ((ae.getSource() == jmiDTOpenEdge) || (ae.getSource() == jmiDROpenEdge)) {
            if (!dataSaved) {
               ...
               int answer = JOptionPane.showConfirmDialog(null, "You currently have unsaved data. Continue?",
                     "Are you sure?", JOptionPane.YES_NO_OPTION);
               if (answer != JOptionPane.YES_OPTION) {
                  logger.info("Aborting due to unsaved data");
                  return;
               }
            }
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION) {
               ...
               logger.info("Created " + tables.length + " table arrays");
               ...
               logger.info("Active file: " + truncatedFilename);
               jfDT.setTitle(DEFINE_TABLES + " - " + truncatedFilename);
               jfDR.setTitle(DEFINE_RELATIONS + " - " + truncatedFilename);
            } else {
               logger.warn("No file chosen, aborting");
               return;
            }
            dataSaved = true;
         }
         ...
         if ((ae.getSource() == jmiDTOpenSave) || (ae.getSource() == jmiDROpenSave)) {
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION) {
               ...
               logger.info("No file chosen, aborting");
               return;
            }
            dataSaved = true;
         }
         ...
         if ((ae.getSource() == jmiDTHelpAbout) || (ae.getSource() == jmiDRHelpAbout)) {
            logger.info("Displaying credits");
