...
public class EdgeConvertFileParser {
   ...
   private boolean validConnectors, isEntity, isAttribute, isUnderlined = false;
   ...
   public EdgeConvertFileParser(File constructorFile) {
      // System.out.println("EdgeConvertFileParser - EdgeConvertFileParser");
      logger.info("Constructing EdgeConvertFileParser\nConstructor file: " + constructorFile);
      numFigure = 0;
      numConnector = 0;
   ...
   public void run(boolean testing) {
      this.openFile(parseFile, testing);
      try {
         if (checkFileType().equals("edge")) { // file is an edge file
            this.parseEdgeFile(testing); // parse the file
            this.makeArrays(); // convert ArrayList objects into arrays of the appropriate Class type
            this.resolveConnectors(testing); // Identify nature of Connector endpoints
         } else if (checkFileType().equals("save")) { // file is a save file
            this.parseSaveFile(); // parse the file
            this.makeArrays(); // convert ArrayList objects into arrays of the appropriate Class type
         } else { // file is some unknown file type
            popup("Unrecognized file format, please check that your file starts with either "
                  + EDGE_ID + " or " + SAVE_ID + " then try again.", testing);
            System.exit(0);
         }
      } catch (IOException ioe) {
         logger.error("IO exception parsing File " + parseFile);
   ...
   public String checkFileType() {
      if (currentLine.startsWith(EDGE_ID)) { // the file chosen is an Edge Diagrammer file
         logger.info("Entered file is Edge file");
         return "edge";
      } else if (currentLine.startsWith(SAVE_ID)) { // the file chosen is a Save file created by this application
         logger.info("Entered file is Save file");
         return "save";
      } else {
         logger.warn("Entered file is incompatible");
         return "invalid";
      }
   }

   public void parseEdgeFile(boolean testing) throws IOException {
      // System.out.println("EdgeConvertFileParser - parseEdgeFile");
      logger.info("Parsing Edge file");
      while ((currentLine = br.readLine()) != null) {
         currentLine = currentLine.trim();
         if (currentLine.startsWith("Figure ")) { // this is the start of a Figure entry
            logger.info("Start of a Figure entry");
            ...
            if (!currentLine.startsWith("Style")) { // this is to weed out other Figures, like Labels
               ...
               if (style.startsWith("Relation")) { // presence of Relations implies lack of normalization
                  popup("The Edge Diagrammer file\n" + parseFile
                        + "\ncontains relations.  Please resolve them and try again.", testing);
                  EdgeConvertGUI.setReadSuccess(false);
                  break;
               }
               if (style.startsWith("Entity")) {
                  logger.info("Figure is entity");
                  isEntity = true;
               }
               if (style.startsWith("Attribute")) {
                  logger.info("Figure is attribute");
                  isAttribute = true;
               }
               ...
               if (text.equals("")) {
                  logger.warn("Blank name found");
                  popup("There are entities or attributes with blank names in this diagram.\nPlease provide names for them and try again.",
                        testing);
                  EdgeConvertGUI.setReadSuccess(false);
                  break;
               }
               ...
               do { // advance to end of record, look for whether the text is underlined
                  ...
                  if (currentLine.startsWith("TypeUnderl")) {
                     logger.info("Text is underlined");
                     isUnderlined = true;
                  }
               ...
               if (isEntity) { // create a new EdgeTable object and add it to the alTables ArrayList
                  if (isTableDup(text)) {
                     ...
                     popup("There are multiple tables called " + text
                           + " in this diagram.\nPlease rename all but one of them and try again.", testing);
                     EdgeConvertGUI.setReadSuccess(false);
                     break;
                  }
         ...
         if (currentLine.startsWith("Connector ")) { // this is the start of a Connector entry
            logger.info("Start of a Connector entry");
            ...
            logger.info(
                  "Connector added.\nNumber: " + numConnector + "\nFirst endpoint: " + endPoint1 + "\nSecond endpoint: "
                        + endPoint2 + "\nFirst end style: " + endStyle1 + "\nSecond end style: " + endStyle2);
         } // if("Connector")
      } // while()
      br.close();
   } // parseEdgeFile()
   ...
   public void parseSaveFile() throws IOException { // this method is unclear and confusing in places
      // System.out.println("EdgeConvertFileParser - parseSaveFile");
      ...
      while (currentLine.startsWith("Table: ")) {
         ...
         logger.info("Table " + numFigure + " found");
      ...
      while ((currentLine = br.readLine()) != null) {
         logger.info("Setting info for table");
   ...
   public void resolveConnectors(boolean testing) { // Identify nature of Connector endpoints
      // System.out.println("EdgeConvertFileParser - resolveConnectors");
      logger.info("Finding Connector endpoints");
      int endPoint1, endPoint2;
      int fieldIndex = 0, table1Index = 0, table2Index = 0;
      validConnectors = true;
      for (int cIndex = 0; cIndex < connectors.length; cIndex++) {
         logger.info("Connectors index " + (cIndex + 1));
         ...
         if (connectors[cIndex].getIsEP1Field() && connectors[cIndex].getIsEP2Field()) { // both endpoints are fields,
                                                                                         // implies lack of
                                                                                         // normalization
            validConnectors = false;
            logger.warn("Both endpoints are fields, attributes may not be normalized");
            popup("The Edge Diagrammer file\n" + parseFile
                  + "\ncontains composite attributes. Please resolve them and try again.", testing);
         ...
         if (connectors[cIndex].getIsEP1Table() && connectors[cIndex].getIsEP2Table()) { // both endpoints are tables
            if ((connectors[cIndex].getEndStyle1().indexOf("many") >= 0) &&
                  (connectors[cIndex].getEndStyle2().indexOf("many") >= 0)) { // the connector represents a many-many
                                                                              ...
                                                                              // normalization
               validConnectors = false;
               logger.warn("Relationship is many to many, tables may not be normalized");
               popup("There is a many-many relationship between tables\n\"" + tables[table1Index].getName()
                     + "\" and \"" + tables[table2Index].getName() + "\""
                     + "\nPlease resolve this and try again.", testing);
               EdgeConvertGUI.setReadSuccess(false); // this tells GUI not to populate JList components
               break; // stop processing list of Connectors
               ...
               logger.info("Figures added to tables");
         ...
         if (fieldIndex >= 0 && fields[fieldIndex].getTableID() == 0) { // field has not been assigned to a table yet
            ...
            validConnectors = false;
            popup("The attribute " + fields[fieldIndex].getName()
   ...
   public void makeArrays() { // convert ArrayList objects into arrays of the appropriate Class type
      // System.out.println("EdgeConvertFileParser - makeArrays");
      logger.info("Making array");
      if (alTables != null) {
   ...
   private boolean isTableDup(String testTableName) {
      // System.out.println("EdgeConvertFileParser - isTableDup");
      logger.info("Checking if " + testTableName + " is a duplicate");
      for (int i = 0; i < alTables.size(); i++) {
         EdgeTable tempTable = (EdgeTable) alTables.get(i);
         if (tempTable.getName().equals(testTableName)) {
            logger.info("Table " + testTableName + " is a duplicate");
            return true;
         }
      }
      logger.info("Table " + testTableName + " is not a duplicate");
      return false;
   }

   public EdgeTable[] getEdgeTables() {
      // System.out.println("EdgeConvertFileParser - getEdgeTables");
      logger.info("Returning edge tables");
      return tables;
   }

   public EdgeField[] getEdgeFields() {
      // System.out.println("EdgeConvertFileParser - getEdgeFields");
      logger.info("Returning edge fields");
      return fields;
   }

   public EdgeConnector[] getEdgeConnectors() {
      logger.info("Returning edge connectors");
      return connectors;
   }

   public boolean getConnectorValidity() {
      // return whether or not the the connectors are valid, or if they violate
      // normalization rules
      logger.info("Returning whether or not the edge connectors are valid: " + validConnectors);
      return validConnectors;
   }

   public ArrayList<EdgeTable> getALTables() {
      logger.info("Returning tables arraylist");
      return alTables;
   }

   public ArrayList<EdgeField> getALFields() {
      logger.info("Returning fields arraylist");
      return alFields;
   }

   public ArrayList<EdgeConnector> getALConnectors() {
      logger.info("Returning connectors arraylist");
      return alConnectors;
   }

   // additional setters, added to make the class less resistant to testing
   public void setEdgeTables(EdgeTable[] tables) {
      logger.info("Artificially setting tables array, for testing");
      this.tables = tables;
   }

   public void setEdgeFields(EdgeField[] fields) {
      logger.info("Artificially setting fields array, for testing");
      this.fields = fields;
   }

   public void setEdgeConnectors(EdgeConnector[] connectors) {
      logger.info("Artificially setting connectors array, for testing");
      this.connectors = connectors;
   }

   public void setALTables(ArrayList<EdgeTable> alTables) {
      logger.info("Artificially setting tables arraylist, for testing");
      this.alTables = alTables;
   }

   public void setALFields(ArrayList<EdgeField> alFields) {
      logger.info("Artificially setting fields arraylist, for testing");
      this.alFields = alFields;
   }

   public void setALConnectors(ArrayList<EdgeConnector> alConnectors) {
      logger.info("Artificially setting connectors arraylist, for testing");
      this.alConnectors = alConnectors;
   }

   // extracting option pane popups to a method, so we can suppress them during
   // testing
   public void popup(String message, boolean test) {
      if (test) { // during tests, we don't want popups we need to close, so just log the message
                  // instead
         logger.info("Testing method, dialog message: " + message);
      } else { // during production, we want the popups to appear for the user
         JOptionPane.showMessageDialog(null, message);
      }
   }

   public void openFile(File inputFile, boolean testing) {
      // System.out.println("EdgeConvertFileParser - openFile");
